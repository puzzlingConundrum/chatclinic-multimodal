...
- Use `cxr_ensemble_tool` after `cxr_classification_tool` when the primary CXR confidence is too low, the top findings are ambiguous, or the model needs a second/third opinion from additional CXR classifiers. The ensemble tool now decides escalation internally: its controller reads the Stage-1 raw confidence scores and, by default, an LLM decides whether to stop after DenseNet (confident) or run the ResNet50 + MedCLIP ensemble (low/ambiguous). Pass `decision_mode=threshold` to force the legacy hard-coded rule for comparison.
